...
class ROLLOUT(object):
    def __init__(self, lstm, update_rate):
        self.lstm = lstm
        self.update_rate = update_rate  # 0.8

        self.num_emb = self.lstm.num_emb # 5000
        self.batch_size = self.lstm.batch_size  # 64
        self.emb_dim = self.lstm.emb_dim  #32
        self.hidden_dim = self.lstm.hidden_dim  #32
        self.sequence_length = self.lstm.sequence_length  #20
        self.start_token = tf.identity(self.lstm.start_token)
        self.learning_rate = self.lstm.learning_rate  #
        print(self.start_token)
        self.g_embeddings = tf.identity(self.lstm.g_embeddings)  # 5000, 32
        self.g_recurrent_unit = self.create_recurrent_unit()  # maps h_tm1 to h_t for generator
        self.g_output_unit = self.create_output_unit()  # maps h_t to o_t (output token logits)

        #####################################################################################################
        # placeholder definition
        self.x = tf.placeholder(tf.int32, shape=[self.batch_size, self.sequence_length]) # sequence of tokens generated by generator 64 , 20
        ...
        self.h0 = tf.zeros([self.batch_size, self.hidden_dim])   # 64, 32
        self.h0 = tf.stack([self.h0, self.h0])   # 2, 64, 32
